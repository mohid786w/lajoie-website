document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('scroll', () => {
        ...
        const propertyCards = document.querySelectorAll('.property-card, .FL-card');
    ...
    document.querySelectorAll('.property-card, .FL-card').forEach(card => {
        card.style.opacity = '0';
        card.style.transform = 'translateY(20px)';
        card.style.transition = 'opacity 0.5s ease, transform 0.5s ease';
    });

    // Initialize section headers with initial hidden state
    document.querySelectorAll('.section-header').forEach(header => {
    ...
    document.querySelectorAll('.property-image, .FL-image').forEach(image => {
